...
const abdraAPI = axios.create({
  baseURL: "https://abd-231006-ra1-2.botics.co",
  headers: {
...
function api_docs_schema_retrieve(payload) {
  return abdraAPI.get(`/api-docs/schema/`, {
    params: {
      lang: payload.lang
    }
  });
...
function modules_two_factor_authentication_enable_2fa_retrieve(payload) {
  return abdraAPI.get(`/modules/two-factor-authentication/enable/2fa`);
}

function modules_two_factor_authentication_enable_2fa_create(payload) {
  return abdraAPI.post(`/modules/two-factor-authentication/enable/2fa`);
}

function modules_two_factor_authentication_enable_2fa_destroy(payload) {
  return abdraAPI.delete(`/modules/two-factor-authentication/enable/2fa`);
}

function modules_two_factor_authentication_google_authenticator_qr_retrieve(payload) {
  return abdraAPI.get(`/modules/two-factor-authentication/google/authenticator/qr`);
}

function modules_two_factor_authentication_send_otp_create(payload) {
  return abdraAPI.post(`/modules/two-factor-authentication/send/otp`);
}

function modules_two_factor_authentication_verify_otp_create(payload) {
  return abdraAPI.post(`/modules/two-factor-authentication/verify/otp`);
}

function modules_two_factor_authentication_verify_otp_create_2(payload) {
  return abdraAPI.post(`/modules/two-factor-authentication/verify/otp/${payload.enable}`);
}
...
function rest_auth_registration_resend_email_create(payload) {
  return abdraAPI.post(`/rest-auth/registration/resend-email/`, payload);
}
